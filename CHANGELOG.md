...
- Removed interfaces for classes wchich supposed to be treated as final, in order to maintain correct integration between Magento2 and Web Components.
    - FieldRolesInterface
...
- Communication layer has been entirely replaced with external communication (library)[https://github.com/FACT-Finder-Web-Components/php-communication-sdk]. 
- Improve Attribute export. User can now select if a given attribute should be exported in a separate column or aggregated in multi attribute column FilterAttributes

### Added
- Add Server Side Rendering support. It could be enabled in module configuration
